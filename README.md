...
This project is not an agent platform; it is a single-app user-facing decision-support tool. My interest in Dedalus is to learn the infra layer that would make agentic versions of this product production-ready.
...
| Translate technical tradeoffs into a product-level "what should we build first" narrative | [Product Decisions](#product-decisions): explicit MVP prioritization and explicitly deprioritized scope, not just a feature list; A future roadmap item is to explore hosting agentic advisory flows on infra like Dedalus, where tool orchestration and reliability would be handled by persistent agent machines rather than a single RAG app. |
