#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Page 3 — Resume + Career Advisor (optimized + weighted skill evidence)

重新定位：CV Fitting Tool 改名為 Resume，AI 建議改稱 Career Advisor —
不是「Ask AI」，而是針對職涯選擇的具體問答（Why this path? / What skills am
I missing? / How different is BI from Product Analytics?）。fit score 計算完
之後會寫進 st.session_state["cv_fit_scores"]，讓 Career Map 頁的 Resume Fit
區塊可以直接讀到同一份分數，不用重算。
"""
...
st.set_page_config(page_title="Resume | Career Overfitter", layout="wide")
...
st.markdown("<h1 style='margin-bottom:0'>📄 Resume</h1>", unsafe_allow_html=True)
st.caption("上傳履歷 → 看 Fit Score → 問 Career Advisor → 找到匹配職缺 → 改履歷")
...
# 讓 Career Map 頁的 Resume Fit 區塊可以直接讀到同一份 fit score，不用重算一次。
st.session_state["cv_fit_scores"] = {
    r["role"]: r["fit_score"] * 100 for r in results if r.get("role")
}
...
st.caption(
    "不是泛用的「Ask AI」，是針對職涯選擇的具體問答 — 例如："
    "Why this path？我適合走 Product Analytics 還是 BI？"
    "我還缺哪些技能？RevOps 跟 Product Analytics 差在哪？"
)
...
if not llm_advisor.is_configured():
    ...
    if st.button("✨ 問 Career Advisor", type="secondary"):
    ...
    if _ai_advice:
        ...
        _ai_best_role = _ai_advice.get("best_fit_role", "—")
        _ai_best_domain = ct.get_domain(_ai_best_role)
        _domain_badge = f"　🧭 {_ai_best_domain}" if _ai_best_domain else ""
        st.markdown(f"**最適職能（Career Advisor 判斷）：{_ai_best_role}**{_domain_badge}")
