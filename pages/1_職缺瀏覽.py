...
st.set_page_config(page_title="職缺瀏覽 | Career Overfitter", layout="wide")
...
# ── Header ───────────────────────────────────────────────
st.markdown("<h1 style='margin-bottom:0'>🔍 職缺瀏覽</h1>", unsafe_allow_html=True)
...
for job in jobs:
    ...
    remote_tag = '<span class="tag" style="background:#f0fdf4;border:1px solid #bbf7d0;color:#15803d;">🌐 Remote</span>' if remote else ""
    ...
    link_btn = f'<a href="{job_url}" target="_blank" style="font-size:0.8rem;color:#1a56db;text-decoration:none;">→ 查看原始職缺</a>' if job_url else ""
    ...
    <div class="job-card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        ...
        <div style="text-align:right;min-width:120px;">
          ...
          <div style="margin-top:6px;">{link_btn}</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
